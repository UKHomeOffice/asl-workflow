...
const {sentToApplicant} = require('../flow/status');
...
const isLicenceHolder = profile => builder => {
  builder
    .whereJsonSupersetOf('data:modelData', {licenceHolderId: profile.id});
};
  ...
  outstanding: (queryBuilder, profile) => {
    queryBuilder
      ...
      .whereIn('status', [
        ...
        builder.where(isLicenceHolder(profile))
          .whereIn('status', [sentToApplicant.id]);
      });
  },
