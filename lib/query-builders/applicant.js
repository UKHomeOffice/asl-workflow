...
module.exports = {
  ...
  // subject = <my-profile-id> AND status NOT IN ("autoresolved", "resolved", "rejected", "withdrawn")
  inProgress: (queryBuilder, profile) => {
    queryBuilder.whereJsonSupersetOf('data', { subject: profile.id })
      .andWhere(builder => {
        builder.whereNotIn('status', [autoResolved.id, resolved.id, rejected.id, withdrawn.id]);
      });
  },
