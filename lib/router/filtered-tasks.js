...
const buildQuery = filters => {
  ...
  const query = Task.query()
    .where({ status })
    .andWhere('updatedAt', '>', moment(start || '2019-07-01').startOf('day').toISOString())
    .andWhere('updatedAt', '<', moment(end || new Date()).endOf('day').toISOString());
  ...
  if (establishments) {
    query.andWhere(filterToEstablishments(establishments));
  }
