  ...
  router.get('/', (req, res, next) => {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        const permCheck = model === 'profile-touched' ? 'profile.relatedTasks' : `${model}.relatedTasks`;

        const params = {
          id: modelId,
          establishment: model === 'establishment' ? modelId : req.query.establishmentId
        };

        console.log('req.user.can', req.user.can);
        console.log('\n', permCheck, params);

        return req.user.can(permCheck, params)
          .then(allowed => {
