...
const hooks = {
  ...
  refused: require('./hooks/refused'),
  representationMade: require('./hooks/make-representations'),
  conditionsAccepted: require('./hooks/accept-conditions')
};
...
module.exports = settings => {
  ...
  flow.hook(`pre-status:*:${sentToApplicant.id}`, hooks.setDeadline(settings));
  flow.hook(`pre-status:*:${sentToApplicant.id}`, hooks.notify(settings));

  flow.hook(`pre-status:*:${conditionsAccepted.id}`, hooks.resolve(settings));
  flow.hook(`pre-status:*:${representationMade.id}`, hooks.representationMade(settings));
  ...
  flow.hook(`status:*:${refused.id}`, hooks.refused(settings));
