...
module.exports = settings => {
  ...
  return model => {
    ...
    let action = get(model, 'data.action');

    // For manage condition tasks we want the action to work as a normal grant when the task is resolved
    if (action === 'manage-conditions') {
      action = 'grant';
      set(model, 'data.action', 'grant');
    }
