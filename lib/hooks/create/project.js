    ...
    switch (action) {
      ...
      case 'reinstate':
        return Promise.resolve()
          ...
          .then(profile => {
            if (!profile.asruUser || !profile.asruInspector) {
              throw new UnauthorisedError('Only inspectors can suspend / reinstate licences');
            }
      ...
      case 'manage-conditions':
        return Promise.resolve()
          .then(() => {
            return messager({ ...model.data, action: 'manage-conditions' });
          })
          .then(changelogModel => {
            // this attached the project version from the fork to the data
            const data = get(model, 'data.data');
            data.version = changelogModel.modelId;
