...
const { updated, recalledByApplicant, discardedByApplicant, discardedByAsru, resubmitted, awaitingEndorsement, withNtco } = require('./status');
...
const isPilEndorsementForOwnPil = (task, profile) => {
  if (task.data.model !== 'pil' || ![awaitingEndorsement.id, withNtco.id].includes(task.status)) {
    return false;
  }
