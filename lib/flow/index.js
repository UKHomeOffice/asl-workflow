...
// licence holder needs to accept the conditions or send representations to the inspector
flow[sentToApplicant.id] = [conditionsAccepted, representationMade];
...
const getNextSteps = c => {
  ...
  if ((c.status === 'with-inspectorate' || c.status === 'representation-made') && action === 'manage-conditions') {
    return [sentToApplicant, discardedByApplicant];
  }
  return nextSteps;
};
