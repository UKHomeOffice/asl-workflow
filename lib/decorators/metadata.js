const Cacheable = require('./cacheable');
const { capitalize } = require('lodash');

module.exports = settings => {

  const { Establishment, Profile } = settings.models;
  const cache = Cacheable();

  return c => {
    const model = settings.models[capitalize(c.data.model)];

    const promises = [
      c.data.establishmentId && cache.query(Establishment, c.data.establishmentId),
      c.data.subject && cache.query(Profile, c.data.subject),
      c.data.changedBy && cache.query(Profile, c.data.changedBy),
      c.data.id && c.data.action === 'delete' && cache.queryWithDeleted(model, c.data.id)
    ];

    return Promise.all(promises)
      .then(([establishment, subject, changedBy, deleted]) => {
        return {
          ...c,
          data: {
            ...c.data,
            establishment,
            subject,
            profile: subject,
            changedBy,
            deleted
          }
        };
      });

  };

};
